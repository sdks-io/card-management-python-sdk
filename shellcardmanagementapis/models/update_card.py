...
from shellcardmanagementapis.models.replace_card_settings import ReplaceCardSettings
...
class UpdateCard(object):
    ...
    Attributes:
        caller (str): The caller to be notified with the status of the update
            ...
            20<br />  Allowed values:<br />  - NextGenUI: This value to be
            used by next gen UI application.<br />  - Motix: This value to be
            used by MOTiX application.<br />  - FleetHubUILifeTime: This value
            to be used by Fleet Hub UI application for life time restriction
            cards.<br />  Note: The values passed in this field are case
            insensitive.
        is_replacement_chargeable (bool): True/False<br />  Optional<br /> 
            When not provided will considered as default value as True.<br /> 
            If passed True, the replacement card will be chargeable, else
        ...
        order_card_replacement (bool): True/False.<br />  Pass True if a
            ...
            damaged card request is successfully placed.<br />  The
            Replacement card request will be processed only when the permanent
            Block card request is successfully processed. <br />  In case of
            damaged card request, the replacement card request will be
            processed immediately.
        card_settings (ReplaceCardSettings): Request entity object for
            ReplaceCardSettings
        reason_id (int): Reason id for updating the card status.  Either
            Reason ID or Text is madatory when TargetStatus is ‘Block’ or
            ‘Damaged’. Else ignored.  Possible values:  1 (Lost)  2 (Stolen) 
            3 (Card no longer required)   <br>When passed, the reason Id will
            be mapped to allowed reason IDs configured for the card type of
            the card. If the given reason Id is not allowed for certain card
            types, then the request will be rejected as invalid ResonId
        ...
        target_status (int): The list of cards passed in ‘Cards’ parameter
            ...
            values –<br />  - TemporaryBlock<br />  - Unblock<br />  -
            Block<br />  - Damaged<br />
    ...
    def from_dictionary(cls,
        ...
        card_settings = ReplaceCardSettings.from_dictionary(dictionary.get('CardSettings')) if 'CardSettings' in dictionary.keys() else APIHelper.SKIP
